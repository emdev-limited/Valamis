learningReport.module('Entities', function(Entities, learningReport, Backbone, Marionette, $, _) {
    ...
    var ReportUsersCollectionService = new Backbone.Service({
        ...
        sync: {
            'read': {
                'path': function (model, options) {
                    var pathPrefix = (options.reportType == Entities.REPORT_TYPES.PATHS) ? 'paths/' : '',
                        courseId = options.courseId || 'all';
                    return path.api.learningReport + pathPrefix + 'course/' + courseId + '/users';
                },
            ...
            'read': {
                'path': function (model, options) {
                    ...
                    if (options.reportType == Entities.REPORT_TYPES.PATHS) {
                        requestPath = path.api.learningReport + 'paths/course/' + options.courseId + '/certificate';
                    } else {
                        requestPath = path.api.learningReport + 'course/' + options.courseId + '/lessons';
                    }
                    return requestPath;
                },
                'method': 'get'
            ...
            'read': {
                'path': function (model, options) {
                    ...
                    if (options.reportType == Entities.REPORT_TYPES.PATHS) {
                        ...
                        requestPath = path.api.learningReport + 'paths/course/' + options.courseId
                            + detailSuffix + '/total';
                    } else {
                        detailSuffix = (!!options.detailId) ? '/' + options.detailId : 's';
                        requestPath = path.api.learningReport + 'course/' + options.courseId
        ...
        sync: {
            'read': {
                'path': function (model, options) {
                    var requestPath = '';
                    if (options.reportType == Entities.REPORT_TYPES.PATHS) {
                        requestPath = path.api.learningReport + 'paths/course/' + Utils.getCourseId()
                            + '/certificate/' + options.itemId + '/goals'
                    } else {
                        requestPath = path.api.learningReport + 'course/' + Utils.getCourseId()
    ...
    Entities.ReportUserPagesModel  = Backbone.Model.extend({
        initialize: function () {
            this.get('pages').forEach( function (page) {
                var legendState = _.find(Entities.LESSON_LEGEND, function (status) { return status.id === page.status });
    ...
    Entities.ReportUserGoalsModel  = Backbone.Model.extend({
        initialize: function () {
            this.get('goals').forEach( function (goal) {
                var legendState = _.find(Entities.PATHS_LEGEND, function (status) { return status.id === goal.status });
    ...
    var ReportSettingsService = new Backbone.Service({
        sync: {
            'read': {
                'path': function (model, options) {
                    return Utils.getContextPath() + 'js/learning-report/defaultSettings.json'
                },
                'data': function (collection, options) {},
                'method': 'get'
            }
            ...
            'updateSettings': {
                'path': function (collection, options) {
                    var settingsType = (options.reportType == Entities.REPORT_TYPES.PATHS)
                        ? 'paths'
                        : 'lessons';
                    return path.root + path.api.learningReport + 'settings/' + settingsType + '/' + options.settingsId
                },
                'data': function (collection, options) {
                    var params = {
                        courseId: Utils.getCourseId(),
        ...
        sync: {
            'read': {
                'path': function (model, options) {
                    ...
                    return path.api.learningReport + pathPrefix + 'course/' + courseId + '/usersCount';
                },
                'data': function (collection, options) {
                    return {
