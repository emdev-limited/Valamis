        ...
        if (agentProperty !== null && agentValue !== null) {
            if (agentProperty === "mbox") {
                ...
                Liferay.Service(
                  ...
                  function () {
                      deferred.reject(new Error("No such liferay user"));
                  }
...
TINCAN.Viewer.prototype.v1QueryObj = function (helper) {
    ...
    jQueryValamis.when(getAgent).done(function(agent) {
...
TINCAN.Viewer.prototype.searchStatements = function () {
    ...
    if (selectVersion === "0.9" || selectVersion === "0.95" || selectVersion === "0.95 + 0.9") {
        ...
        var that = this;
        jQueryValamis.when(this.v1QueryObj(helper)).done(function(queryObj) {
...
TINCAN.Viewer.prototype.statementsFetched = function (multiStream) {
    ...
    jQueryValamis("#showAllStatements").toggle(moreUrl != undefined && moreUrl != "");
};
...
TINCAN.Viewer.prototype.pageInitialize = function () {
    ...
    jQueryValamis("#export").click(
        function () {
            jQueryValamis("#exportLoading").show();
            jQueryValamis("#export").hide();
            jQueryValamis("#cancel").show();
            jQueryValamis('#linkCSV').hide();
            jQueryValamis('#linkJSON').hide();
            jQueryValamis('#status').html("");

            var count = jQueryValamis("#exportCount").val();
            var helper = new TINCAN.Viewer.prototype.TinCanSearchHelper();

            var actor = helper.getActor();
            var activity = helper.getActivity();
            var verb = helper.getVerb();
            var format = helper.getFormat();
            var since = helper.getSince();
            var until = helper.getUntil();
            var registration = helper.getRegistration();
            var related_agents = helper.getRelatedAgents();
            var related_activities = helper.getRelatedActivities();
            ...
            if (related_agents !== null) {
                queryObj.related_agents = related_agents;
            }

            if (related_activities !== null) {
                queryObj.related_activities = related_activities;
            }
            ...
            jQueryValamis.ajax({
                ...
                url: path.root + path.api.exportData + 'export?count=' + count + '&' + str + '&courseId=' + Utils.getCourseId(),
                data: {
                    'p_auth': Liferay.authToken
                },
                success: function (data) {
                    currentGuid = data;
                    intervalId = setInterval(doGetCSVStatus, 5000);
                }
            });
        }
    );

    jQueryValamis("#cancel").click(
        function () {
            ...
            jQueryValamis.ajax({
                ...
                url: path.root + path.api.exportData + 'cancel?guid=' + currentGuid + '&courseId=' + Utils.getCourseId(),
                data: {
                    'p_auth': Liferay.authToken
                },
                success: function (data) {
                    if(data == 'ok') {
                        jQueryValamis("#exportLoading").hide();
                        jQueryValamis("#export").show();
                        jQueryValamis("#cancel").hide();
                        jQueryValamis('#status').html("");
    ...
    function doGetCSVStatus() {
        jQueryValamis.ajax({
            ...
            url: path.root + path.api.exportData + 'status?guid=' + currentGuid + '&courseId=' + Utils.getCourseId(),

            success: function (data) {
                var res = JSON.parse(data);
                if(res.isFinished) {
                    jQueryValamis("#exportLoading").hide();
                    jQueryValamis('#linkCSV').attr('href', '/delegate/export/csv/statements?name='+res.linkCSV + '&courseId=' + Utils.getCourseId());
                    jQueryValamis('#linkCSV').show();
                    jQueryValamis('#linkJSON').attr('href', '/delegate/export/json/statements?name='+res.linkJSON + '&courseId=' + Utils.getCourseId());
                    jQueryValamis('#linkJSON').show();
                    jQueryValamis("#export").show();
                    jQueryValamis("#cancel").hide();
                    jQueryValamis('#status').html(res.data);
                    if (intervalId){
                        clearInterval(intervalId);
                        intervalId = undefined;
                    }
