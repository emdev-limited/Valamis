...
def lfPersistence = Settings.liferay.version match {
  case Settings.Liferay620.version => lfPersistence620
}

def hook = Settings.liferay.version match {
  case Settings.Liferay620.version => hookLf620
}
...
lazy val gradebook = (project in file("valamis-gradebook"))
  ...
  .settings(libraryDependencies ++= Settings.liferay.dependencies)
  .dependsOn(core, questionbank, scormLesson, lesson, lfService)
...
lazy val slickPersistence = (project in file("valamis-slick-persistence"))
  ...
  .settings(libraryDependencies ++= Dependencies.slick ++ Seq(Libraries.javaxInject))
  .settings(parallelExecution in test := false)
  .dependsOn(core, certificate, social, slide)
...
lazy val slide = (project in file("valamis-slide"))
  ...
  .settings(libraryDependencies ++= Settings.liferay.dependencies ++
    Seq(Libraries.apachePDF, Libraries.poiScratchPad,
      Libraries.apacheXml
...
lazy val persistenceApi = (project in file("learn-persistence-api"))
  ...
  .settings(libraryDependencies ++= Settings.liferay.dependencies)
  .dependsOn(core, lesson, lrssupport)
...
lazy val hookLf620 = (project in file("valamis-hook"))
  ...
  .settings(postProcess in webapp := { webappDir =>
    ...
    val propertiesContent = IO.read(webappDir / "../../../valamis-hook/src/main/resources/liferay-plugin-package.properties")
