...
abstract class ServletBase
  ...
  error {
    ...
    case e: ParseException                  => halt(HttpServletResponse.SC_BAD_REQUEST, e.getMessage)
    case e: SizeConstraintExceededException => RequestEntityTooLarge("too much!")
    case e: LMustBeAuthenticatedException   => halt(HttpServletResponse.SC_FORBIDDEN, reason = e.getMessage)
