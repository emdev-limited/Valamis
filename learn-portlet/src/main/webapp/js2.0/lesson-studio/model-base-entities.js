...
var baseLessonStudioModels = {
    LessonPageModel: ModelWithDevicesProperties.extend({
              ...
              .on('add change', function(model, collection, options){
                  ...
                  if(slidesApp.historyManager && !slidesApp.initializing){
                      slidesApp.historyManager.pushModelChange(model, _.extend({
                          skipAttributes: ['id','tempId','slideElements','height','isTemplate','isLessonSummary'],
                          omitUndoActions: ['height'],
                          namespace: 'slides'
                      }, options));
                  }
              });
        ...
        changeElementsFont: function(){
            var slideId = this.get('id') || this.get('tempId'),
              font = this.getFont();
            var elements = slidesApp.slideElementCollection.where({ slideId: slideId, toBeRemoved: false });
            _.each(elements, function(model){
                if( model.get('slideEntityType') == 'question'){
                ...
                if(_.contains(['text','plaintext'], model.get('slideEntityType')) ){
                    model.resetProperties({
                        fontSize: font.fontSize
                    });
                }
    ...
    LessonPageElementModel: ModelWithDevicesProperties.extend({
        ...
        initialize: function(){
            ...
            this.on('add change',function(model, collection, options){
                ...
                if(slidesApp.historyManager && !slidesApp.initializing){
                    slidesApp.historyManager.pushModelChange(model, _.extend({
                        skipAttributes: ['id','tempId','googleClientApiAvailable','active','selected'].concat(this.devicesProperties),
                        omitUndoActions: ['googleClientApiAvailable','active','selected', 'id','tempId'].concat(this.devicesProperties),
