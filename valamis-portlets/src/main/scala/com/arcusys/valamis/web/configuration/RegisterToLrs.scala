...
import com.arcusys.learn.liferay.LiferayClasses.{LMessage, LMessageListener, LSimpleAction}
...
class LrsRegistrator(implicit val bindingModule: BindingModule) extends Injectable {
  ...
  def register(): Unit = {
    ...
    if (endpointService.getEndpoint.isDefined) {
      logger.info("Lrs already registered, companyId: " + CompanyHelper.getCompanyId)
    }
    else {
