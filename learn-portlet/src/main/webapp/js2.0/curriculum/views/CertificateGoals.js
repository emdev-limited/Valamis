function getGoalUrlPostfix(goalType) {
  ...
  switch (goalType) {
    case GOAL_TYPE.COURSE:
      p = '/course';
      break;
    case GOAL_TYPE.STATEMENT:
      p = '/statement';
      break;
    case GOAL_TYPE.ACTIVITY:
      p = '/activity';
      break;
    case GOAL_TYPE.PACKAGE:
      p = '/package';
      break;
    default:
...
GoalService = new Backbone.Service({
    url: path.root,
    sync: {
        'update': {
            'path': function(model){
              return path.api.certificates + model.get('certificateID') + getGoalUrlPostfix(model.get('type'));
            },
            'data': function (model) {
...
var GoalView = Backbone.View.extend({
    ...
    initialize: function (options) {
        if (options.type == GOAL_TYPE.COURSE) {
            this.$el = jQuery('<li>');
            this.$el.attr('id', "courseSortableItem_" + this.model.get('courseGoalId'));
        }
        else {
            this.$el = jQuery('<tr>');
        }
    ...
    render: function () {
        ...
        if (this.model.get('type') == GOAL_TYPE.COURSE && !this.isPublished)
          templateName = "#sortableCourseRowTemplate";
        ...
        this.$el.find("#certificateCoursesTable").sortable({
            ...
            stop: jQuery.proxy(function (event, ui) {
                ...
                var ids = jQuery.param({
                    'courseGoalIds': sortedAnswers.map(function (i) {
                        return i.replace('courseSortableItem_', '');
                    })
                }, true);

                window.LearnAjax.post(path.root + path.api.certificates + this.model.id + "/do/move_course" + "?courseId=" + Utils.getCourseId() + '&' + ids);
            }, this)
