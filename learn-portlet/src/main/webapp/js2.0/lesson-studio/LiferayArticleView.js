var articleCollectionService = new Backbone.Service({
    url: path.root + path.portlet.prefix,
    sync: {
        'read': {
            'path': function () {
                return 'services/liferay/article?companyID=' + Liferay.ThemeDisplay.getCompanyId();
            },
            'method': 'get'
        }
    }
...
var articleService = new Backbone.Service({
    ...
    targets: {
        'getContent': {
            'path': function (model) {
                return '&companyID=' + Liferay.ThemeDisplay.getCompanyId()
