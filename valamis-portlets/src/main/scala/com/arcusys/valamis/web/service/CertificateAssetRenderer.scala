...
import javax.portlet.{PortletRequest, PortletResponse, RenderRequest, RenderResponse}
...
import com.arcusys.learn.liferay.util.{HtmlUtilHelper, PortalUtilHelper, PortletName}
...
class CertificateAssetRenderer(cert: Certificate) extends LBaseAssetRenderer {
  ...
  override def getURLEdit(liferayPortletRequest: LLiferayPortletRequest,
    ...
    val portletURL = liferayPortletResponse.createLiferayPortletURL(
      getControlPanelPlid(liferayPortletRequest), PortletName.CertificateViewer.key, PortletRequest.RENDER_PHASE)
  ...
  def getAssetObject: AnyRef = assetEntry

  def getSummary(portletRequest: PortletRequest, portletResponse: PortletResponse): String = HtmlUtilHelper.stripHtml(assetEntry.getSummary)

  def include(httpServletRequest: HttpServletRequest, httpServletResponse: HttpServletResponse, s: String): Boolean = false
}
